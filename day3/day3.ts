...
const getCoordsForMovement = (position: Coord, movement: Move) => {
  const { x, y } = position;
  const coords: Coord[] = [];

  // @ts-ignore
  const direction = movement[0];
  const amount = movement.slice(1);

  if (['R', 'L'].includes(direction)) {
    // changing x
    for (let i = 1; i <= parseInt(amount, 10); i++) {
      const nextX = direction === 'R' ? x + i : x - i;
      coords.push({ x: nextX, y });
    }
  } else {
    // changin y
    for (let i = 1; i <= parseInt(amount, 10); i++) {
      const nextY = direction === 'U' ? y + i : y - i;
      coords.push({ x, y: nextY });
    }
  }
  return coords;
};
...
const followPath = (path) => {
  ...
  for (let move of moves) {
    const currentCoord: Coord = coords[coords.length - 1];
    // @ts-ignore
    coords.push(...getCoordsForMovement(currentCoord, move));
  }
  return coords;
};

const findIntersections = (coords1, coords2) => {
  const intersections: Array<Array<Coord | number>> = [];
  console.log(coords1.length);
  console.log(coords2.length);

  for (let i = 0; i < coords1.length; i++) {
    for (let j = 0; j < coords2.length; j++) {
      const { x: x1, y: y1 } = coords1[i];
      const { x: x2, y: y2 } = coords2[j];
      if (x1 === x2 && y1 === y2) {
        console.log('pushing', x1, y1, i, j);
        intersections.push([coords1[i], i, j]);
      }
    }
  }
...
const findShortestDist = (intersections: Coord[]) => {
  let shortestDist = -1;
...
const findFirstIntersection = (intersections: Array<Array<Coord | number>>) => {
  let shortestSteps = [-1, -1];
  for (let intersection of intersections) {
    // x is 0, y 1
    const [ dontneed, first, second ] = intersection;
    if (first == 0 && second == 0) { continue; }
    if (first < shortestSteps[0] || shortestSteps[0] < 0) {
      shortestSteps[0] = first as number;
    }
    if (second < shortestSteps[1] || shortestSteps[1] < 0) {
      shortestSteps[1] = second as number;
    }
  }
...
const doTheThing = (paths: string[]) => {
  const [coords1, coords2] = [ followPath(paths[0]), followPath(paths[1])];
  const intersections: Array<Array<Coord | number>> = findIntersections(coords1, coords2);
  // @ts-ignore
  const shortestDist = findShortestDist(intersections.map(([i]) => i));
  const shortestSteps = findFirstIntersection(intersections);
  return [shortestDist, shortestSteps];
};

const test1 = [ "R8,U5,L5,D3", "U7,R6,D4,L4" ];
const test2 = [ "R75,D30,R83,U83,L12,D49,R71,U7,L72", "U62,R66,U55,R34,D71,R55,D58,R83" ];
const test3 = [ "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51", "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7" ];
const real = [
...
for (let test of [ /* test1 , test2, test3,*/ real ]) {
  console.log(doTheThing(test));
}
