...
void print(std::vector<Coord> arr) {
...
std::vector<Coord> getCoordsForMovement(Coord position, std::string movement) {
    const int x = position.x;
    const int y = position.y;
    const char direction = movement[0];
    const int amount = std::stoi(movement.substr(1));
    std::vector<Coord> coords;
    if (direction == 'R' || direction == 'L') {
        for (int i = 1; i <= amount; i++) {
            const int nextX = direction == 'R' ? x + i : x - i;
            Coord nextCoord;
            nextCoord.x = nextX;
            nextCoord.y = y;
        ...
        for (int i = 1; i <= amount; i++) {
            const int nextY = direction == 'U' ? y + i : y - i;
            Coord nextCoord;
            nextCoord.x = x;
            nextCoord.y = nextY;
            coords.push_back(nextCoord);
...
std::vector<Coord> followPath(std::string path) {
    ...
    for (std::string movement : parts) {
        Coord current = coords[coords.size() - 1];
        std::vector<Coord> newCoords = getCoordsForMovement(current, movement);
...
int getShortestDist(const std::vector<Coord> &intersections) {
    int shortestDist = -1;
