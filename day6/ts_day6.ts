...
const test1 = [ 'COM)B', 'B)C', 'C)D', 'D)E', 'E)F', 'B)G', 'G)H', 'D)I', 'E)J', 'J)K', 'K)L', ];
const test2 = [ 'COM)B', 'B)C', 'C)D', 'D)E', 'E)F', 'B)G', 'G)H', 'D)I', 'E)J', 'J)K', 'K)L', 'K)YOU', 'I)SAN', ];

const allOrbits = {};

function traceOrbit(orbits, input) {
  let next = input;
  let numOrbits = 0;
  while (orbits[next] !== undefined) {
    next = orbits[next];
    numOrbits++;
  }
  return numOrbits;
...
function drawOrbit(orbits, input) {
  let next = input;
  const path = [];
  while (orbits[next] !== undefined) {
    path.push(orbits[next]);
    next = orbits[next];
  }
  return path;
}
...
// get path for YOU and SAN
const myPath = drawOrbit(allOrbits, 'YOU');
const santaPath = drawOrbit(allOrbits, 'SAN');

// find all intersections?
const shared = getIntersections(myPath, santaPath);
// get num from me and num from santa
const myDistFromShared = myPath.findIndex(val => val === shared);
const santaDistFromShared = santaPath.findIndex(val => val === shared);

const val = myDistFromShared + santaDistFromShared;
console.log('VALUE:', val);

/*
console.log('myDist', myDistFromShared);
console.log('santaDist', santaDistFromShared);
console.log('shared', shared);

console.log('me', myPath);
console.log('santa', santaPath);
/* uncomment for sum
const nums = Object.keys(allOrbits).map(o => traceOrbit(allOrbits, o));
